...
@ck.option('--output-root', '-or', default='output', help='Root folder for outputs')
@ck.option('--fold', '-f', default=None, help='Fold number or cluster name')

def main(in_file, data_root, threshold, batch_size, device):

    # Extract ESM features
    fn = os.path.splitext(in_file)[0]
    # TODO custom named fold# directories
    if fold is not None:
        fold_dir = Path(output_root) / f"fold{fold}"
    else:
        fold_dir = Path(output_root) / "run"
    ...
    for ont in ['mf', 'cc', 'bp']:
        ...
        out_file = fold_dir / f'{fn}_preds_{ont}.tsv.gz'
        ...
        #TODO write output to specific folder
