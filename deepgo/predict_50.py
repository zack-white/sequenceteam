...
# Predict.py modified from default threshold = 0.1.
...
@ck.option('--threshold', '-t', default=0.5, help='Prediction threshold')
@ck.option('--batch-size', '-bs', default=32, help='Batch size for prediction model')
#TODO changed default from CPU to GPU cuda 2
@ck.option(
...
def main(in_file, data_root, threshold, batch_size, device):
    ...
    # Replaced with a check to see if the esm.pkl file has already been generated.
    # Extract or load ESM.pkl dictionary
    # 
    fn = os.path.splitext(in_file)[0]
    out_file_esm = f'{fn}_esm.pkl'

    if os.path.exists(out_file_esm):
        print(f"Loading existing embeddings: {out_file_esm}")
        esm_dict = th.load(out_file_esm)          # load as dict
        proteins = esm_dict['proteins']           # string IDs
        data = th.tensor(np.vstack(esm_dict['data']))  # convert list of embeddings to tensor
    else:
    ...
    for ont in ['mf', 'cc', 'bp']:
        ...
        out_file = f'{Path(fn).name}_preds_{ont}.tsv.gz'
